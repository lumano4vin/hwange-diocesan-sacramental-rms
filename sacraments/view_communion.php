 ...
 * First Holy Communion Record Detailed View - Modernized
 */
...
$record = db_fetch("
    ...
    WHERE c.communion_id = ?
", [$id]);

if (!$record) die("Canonical record not found.");

// Header metadata
$header_title = "First Communion Archive Entry";
...
<head>
    ...
    <title>View First Communion - <?php echo h($record['first_name']); ?> - Hwange Diocesan RMS</title>
        ...
        <main class="main-content">
            ...
            <div class="content-body" style="padding: 2rem 0;">
                ...
                <div class="action-bar" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 2rem;">
                    <a href="communion_list.php" class="btn btn-secondary" style="display: flex; align-items: center; gap: 8px;">
                        <ion-icon name="arrow-back-outline"></ion-icon> Back to Registry
                    </a>
                    <div style="display: flex; gap: 1rem;">
                        <a href="communion_certificate.php?id=<?php echo $id; ?>" target="_blank" class="btn btn-primary" style="background: var(--accent); color: #000; font-weight: 800; display: flex; align-items: center; gap: 8px; border: none;">
                            <ion-icon name="print-outline"></ion-icon> Print Certificate
                        </a>
                        <a href="communion_edit.php?id=<?php echo $id; ?>" class="btn btn-secondary" style="display: flex; align-items: center; gap: 8px;">
                    ...
                    <div class="main-details">
                        <div class="card bg-card" style="padding: 3rem; border-radius: 2rem; border: 1px solid rgba(255,255,255,0.05); backdrop-filter: blur(20px);">
                            <h3 style="font-family: 'Outfit', sans-serif; font-size: 1.5rem; color: white; margin-bottom: 2rem; display: flex; align-items: center; gap: 12px;">
                                <ion-icon name="nutrition-outline" style="color: var(--accent);"></ion-icon>
                            ...
                            <div class="details-rows" style="display: flex; flex-direction: column; gap: 2rem;">
                                <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 2rem;">
                                    ...
                                    <div>
                                        <span style="display: block; font-size: 0.7rem; color: var(--text-muted); text-transform: uppercase; letter-spacing: 1px; margin-bottom: 4px;">Date of First Communion</span>
                                        <span style="font-size: 1.2rem; font-weight: 700; color: white;"><?php echo date('d F, Y', strtotime($record['date_of_communion'])); ?></span>
                                    </div>
                                    <div>
                                        ...
                                        <span style="font-size: 1.2rem; font-weight: 700; color: var(--accent);"><?php echo get_age($record['dob'], $record['date_of_communion']); ?></span>
                                    </div>
                                </div>

                                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 2rem;">
                                    <div>
                                        <span style="display: block; font-size: 0.7rem; color: var(--text-muted); text-transform: uppercase; letter-spacing: 1px; margin-bottom: 4px;">Parish of Celebration</span>
                                        <span style="font-size: 1.1rem; color: white;"><?php echo h($record['parish_name']); ?></span>
                                    </div>
                                    <div>
                                        <span style="display: block; font-size: 0.7rem; color: var(--text-muted); text-transform: uppercase; letter-spacing: 1px; margin-bottom: 4px;">Minister</span>
                                        <span style="font-size: 1.1rem; color: white;"><?php echo h($record['minister_name']); ?></span>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="side-metadata">
                        ...
                        <div class="card bg-card" style="padding: 2rem; border-radius: 1.5rem; border: 1px solid rgba(255,255,255,0.05);">
                            ...
                            <div style="text-align: center;">
                                <div style="font-size: 3rem; color: var(--accent); margin-bottom: 10px;"><ion-icon name="shield-checkmark"></ion-icon></div>
                                <span style="display: block; font-weight: 800; color: var(--accent); text-transform: uppercase; font-size: 0.8rem; margin-bottom: 15px;">Sacramental Record Valid</span>
                                <div style="font-family: monospace; font-size: 0.6rem; color: var(--text-muted); word-break: break-all; background: rgba(0,0,0,0.2); padding: 10px; border-radius: 8px;">
                                    <?php echo h($record['verification_hash'] ?: 'COMM_HASH_VERIFICATION_SECURE'); ?>
                                </div>
                            </div>
                        </div>
                    </div>

 ...
 * Register First Holy Communion - Modernized & Backend Polished
 */
...
// Header metadata
$header_title = "Register First Communion";
$header_subtitle = "Enter canonical first holy communion details into the Diocesan archives.";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($person_id && $parish_id && $date_of_communion) {
        ...
        try {
            $sql = "INSERT INTO first_holy_communions (person_id, parish_id, date_of_communion, minister_name, register_book_number, page_number, entry_number, verification_hash, guid, diocese_id) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            
            db_query($sql, [$person_id, $parish_id, $date_of_communion, $minister_name, $register_book, $page_number, $entry_number, $verification_hash, $guid, $diocese_id]);
            
            $new_id = db_fetch("SELECT last_insert_rowid() as id")['id'];
            log_audit($_SESSION['user_id'], 'CREATE', 'first_holy_communions', $new_id, "Registered first communion for person_id: $person_id");
            
            set_flash("First Holy Communion record has been successfully registered.");
...
<head>
    ...
    <title>Register First Communion - Hwange Diocesan RMS</title>
        ...
        <main class="main-content">
            ...
            <div class="content-body" style="padding: 2rem 0; max-width: 900px; margin: 0 auto;">
                ...
                <div class="action-bar" style="margin-bottom: 2rem;">
                    <a href="communion_list.php" class="btn btn-secondary" style="display: flex; width: max-content; align-items: center; gap: 8px;">
                    ...
                    <form action="" method="POST" class="premium-form">
                        ...
                        <div class="form-section" style="margin-bottom: 3rem;">
                            <h3 style="font-family: 'Outfit', sans-serif; font-size: 1.25rem; color: var(--accent); margin-bottom: 1.5rem; display: flex; align-items: center; gap: 10px;">
                                <ion-icon name="nutrition-outline"></ion-icon> Primary Subject Information
                            </h3>
                        ...
                        <div class="form-section" style="margin-bottom: 3rem;">
                            ...
                            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem;">
                                <div class="form-group">
                                    <label style="display: block; font-size: 0.75rem; color: var(--text-muted); text-transform: uppercase; letter-spacing: 1px; margin-bottom: 8px;">Date of Communion *</label>
                                    <input type="date" name="date_of_communion" required style="width: 100%; padding: 1rem; background: rgba(0,0,0,0.3); border: 1px solid rgba(255,255,255,0.1); border-radius: 12px; color: white;">
                                </div>
                                <div class="form-group">
                                    <label style="display: block; font-size: 0.75rem; color: var(--text-muted); text-transform: uppercase; letter-spacing: 1px; margin-bottom: 8px;">Minister (Priest)</label>
                                    <input type="text" name="minister_name" placeholder="e.g. Fr. J. Moyo" style="width: 100%; padding: 1rem; background: rgba(0,0,0,0.3); border: 1px solid rgba(255,255,255,0.1); border-radius: 12px; color: white;">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="form-footer" style="padding-top: 2rem; border-top: 1px solid rgba(255,255,255,0.05); display: flex; justify-content: flex-end; gap: 1rem;">
                            <button type="submit" class="btn btn-primary" style="background: var(--accent); color: #000; font-weight: 800; padding: 1.25rem 3rem; border-radius: 1rem; border: none; font-size: 1.1rem; box-shadow: 0 10px 30px rgba(251, 191, 36, 0.3);">Register Archive Entry</button>
                        </div>
                    </form>

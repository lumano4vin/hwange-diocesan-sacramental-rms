...
$id = $_GET['id'] ?? null;
if (!$id) {
    set_flash("Confirmation ID is required for editing.", "error");
    redirect("confirmation_list.php");
}

$record = db_fetch("SELECT * FROM confirmations WHERE confirmation_id = ?", [$id]);
...
if (!has_record_permission($record, 'edit')) {
    ...
    redirect("view_confirmation.php?id=" . $id);
}

$header_title = "Edit Confirmation Record";
$header_subtitle = "Update canonical confirmation details.";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($person_id && $parish_id && $date_of_confirmation) {
        try {
            ...
            db_query($sql, [$person_id, $parish_id, $date_of_confirmation, $minister, $sponsor, $register_book, $page_number, $entry_number, $status, $id]);
            
            log_audit($_SESSION['user_id'], 'UPDATE', 'confirmations', $id, "Updated confirmation record for id: $id");
            
            set_flash("Confirmation record updated successfully.");
            redirect("view_confirmation.php?id=" . $id);
        } catch (Exception $e) {
...
<head>
    ...
    <title>Edit Confirmation - Hwange Diocesan RMS</title>
        ...
        <main class="main-content">
            ...
            <div class="content-body" style="padding: 2rem 0; max-width: 900px; margin: 0 auto;">
                ...
                <div class="action-bar" style="margin-bottom: 2rem;">
                    <a href="view_confirmation.php?id=<?php echo $id; ?>" class="btn btn-secondary" style="display: flex; width: max-content; align-items: center; gap: 8px;">
                    ...
                    <form action="" method="POST" class="premium-form">
                        ...
                        <div class="form-section" style="margin-bottom: 3rem;">
                            <h3 style="font-family: 'Outfit', sans-serif; font-size: 1.25rem; color: #fbbf24; margin-bottom: 1.5rem; display: flex; align-items: center; gap: 10px;">
                                <ion-icon name="flame-outline"></ion-icon> Candidate Information
                            </h3>
                            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem;">
                                ...
                                <div class="form-group">
                                    <label>Parish of Confirmation *</label>
                        ...
                        <div class="form-section" style="margin-bottom: 3rem;">
                            ...
                            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem; margin-bottom: 1.5rem;">
                                <div class="form-group">
                                    ...
                                    <input type="date" name="date_of_confirmation" required value="<?php echo h($record['date_of_confirmation']); ?>" class="input-field">
                                </div>
                                <div class="form-group">
                                    <label>Minister of Sacraments (Bishop/Priest)</label>
                                    <input type="text" name="minister" value="<?php echo h($record['minister']); ?>" class="input-field">
                                </div>
                            </div>
                            <div class="form-group">
                                <label>Confirmation Sponsor</label>
                                <input type="text" name="sponsor" value="<?php echo h($record['sponsor']); ?>" class="input-field">
                            </div>
                        </div>
                        ...
                        <div class="form-footer" style="padding-top: 2rem; border-top: 1px solid rgba(255,255,255,0.05); display: flex; justify-content: flex-end; gap: 1rem;">
                            <button type="submit" class="btn btn-primary" style="background: #fbbf24; color: #000; font-weight: 800; padding: 1.25rem 3rem; border-radius: 1rem; border: none; font-size: 1.1rem;">Save Confirmation Updates</button>
                        </div>
                    </form>

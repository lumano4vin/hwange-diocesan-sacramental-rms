 ...
 * Register Holy Orders (Ordination)
 */
...
// Header metadata
$header_title = "Register Holy Orders";
$header_subtitle = "Record ordination to the Diaconate, Priesthood, or Episcopate.";

$pdo = getDB();
// Fetch parishioners and parishes for dropdowns (Ordinands must be Male and Active/Moved but not Deceased)
$parishioners = db_fetchAll("SELECT person_id, first_name, last_name FROM parishioners WHERE status != 'Deceased' AND gender = 'Male' ORDER BY last_name, first_name");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($person_id && $parish_id && $event_date && $record_type) {
        ...
        try {
            ...
            // Update title of the parishioner to Rev. or Rev. Fr. or Most Rev.
            $new_title = 'Rev.';
            if ($record_type === 'Priesthood') {
                $new_title = 'Rev. Fr.';
            } elseif ($record_type === 'Episcopate') {
                $new_title = 'Most Rev.';
            }
            ...
            if ($baptism) {
                ...
                $details = "Ordained $record_type. Minister: $celebrant_superior. Place: $place";
            ...
            log_audit($_SESSION['user_id'], 'CREATE', 'ordinations_professions', $new_id, "Registered Holy Orders ($record_type) for person_id: $person_id");
            
            set_flash("Holy Orders (Ordination) has been successfully registered. Subject title updated to $new_title.");
...
<head>
    ...
    <title>Register Holy Orders - Hwange Diocesan RMS</title>
            ...
            <div class="content-body" style="padding: 2rem 0; max-width: 900px; margin: 0 auto;">
                ...
                <div class="card bg-card" style="padding: 3rem; border-radius: 2rem; border: 1px solid rgba(255,255,255,0.05); backdrop-filter: blur(20px);">
                    ...
                    <form action="" method="POST" class="premium-form">
                        ...
                        <div class="form-section" style="margin-bottom: 3rem;">
                            ...
                            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem;">
                                <div class="form-group">
                                    <label style="display: block; font-size: 0.75rem; color: var(--text-muted); text-transform: uppercase; letter-spacing: 1px; margin-bottom: 8px;">Faithful Subject (Male) *</label>
                                    ...
                                    <div style="margin-top: 10px; font-size: 0.8rem;">
                                        ...
                                        <a href="../parishioners/add.php?ref=orders" style="color: #34d399; text-decoration: none; font-weight: 700; margin-left: 5px;">+ Register New Person First</a>
                                    </div>
                                </div>
                                <div class="form-group">
                                    <label style="display: block; font-size: 0.75rem; color: var(--text-muted); text-transform: uppercase; letter-spacing: 1px; margin-bottom: 8px;">Order Rank *</label>
                                    <select name="record_type" required style="width: 100%; padding: 1rem; background: rgba(0,0,0,0.3); border: 1px solid rgba(255,255,255,0.1); border-radius: 12px; color: white;">
                                        <option value="Diaconate">Diaconate (Ordination)</option>
                                        <option value="Priesthood">Priesthood (Ordination)</option>
                                        <option value="Episcopate">Episcopate (Consecration)</option>
                                    </select>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="form-section" style="margin-bottom: 3rem;">
                            <h3 style="font-family: 'Outfit', sans-serif; font-size: 1.25rem; color: #34d399; margin-bottom: 1.5rem; display: flex; align-items: center; gap: 10px;">
                                <ion-icon name="ribbon-outline"></ion-icon> Ordination Details
                            </h3>
                            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem; margin-bottom: 1.5rem;">
                                <div class="form-group">
                                    <label style="display: block; font-size: 0.75rem; color: var(--text-muted); text-transform: uppercase; letter-spacing: 1px; margin-bottom: 8px;">Date of Ordination *</label>
                                ...
                                <div class="form-group">
                                    <label style="display: block; font-size: 0.75rem; color: var(--text-muted); text-transform: uppercase; letter-spacing: 1px; margin-bottom: 8px;">Ordaining Bishop *</label>
                                    <input type="text" name="celebrant_superior" required placeholder="The Most Rev. Bishop Name" style="width: 100%; padding: 1rem; background: rgba(0,0,0,0.3); border: 1px solid rgba(255,255,255,0.1); border-radius: 12px; color: white;">
                                </div>
                            </div>
                            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem; margin-bottom: 1.5rem;">
                                <div class="form-group">
                                    <label style="display: block; font-size: 0.75rem; color: var(--text-muted); text-transform: uppercase; letter-spacing: 1px; margin-bottom: 8px;">Parish of Ordination *</label>
                                ...
                                <div class="form-group">
                                    <label style="display: block; font-size: 0.75rem; color: var(--text-muted); text-transform: uppercase; letter-spacing: 1px; margin-bottom: 8px;">Specific Place / Cathedral</label>
                                    <input type="text" name="place" placeholder="e.g. St. Ignatius Cathedral" style="width: 100%; padding: 1rem; background: rgba(0,0,0,0.3); border: 1px solid rgba(255,255,255,0.1); border-radius: 12px; color: white;">
                                </div>
                            </div>
                            <div class="form-group">
                                <label style="display: block; font-size: 0.75rem; color: var(--text-muted); text-transform: uppercase; letter-spacing: 1px; margin-bottom: 8px;">Congregation / Incardination (if any)</label>
                                <input type="text" name="congregation" value="Hwange Diocese" placeholder="e.g. OFM Capuchin, Hwange Diocese" style="width: 100%; padding: 1rem; background: rgba(0,0,0,0.3); border: 1px solid rgba(255,255,255,0.1); border-radius: 12px; color: white;">
                            </div>
                        </div>
                        ...
                        <div class="form-footer" style="padding-top: 2rem; border-top: 1px solid rgba(255,255,255,0.05); display: flex; justify-content: flex-end; gap: 1rem;">
                            <button type="submit" class="btn btn-primary" style="background: #34d399; color: #000; font-weight: 800; padding: 1.25rem 3rem; border-radius: 1rem; border: none; font-size: 1.1rem; box-shadow: 0 10px 30px rgba(52, 211, 153, 0.3);">Register Holy Orders</button>
                        </div>
                    </form>

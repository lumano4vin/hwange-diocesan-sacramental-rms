 ...
 * Confirmation Records List - Modernized
 */
...
// Header metadata
$header_title = "Confirmation Registry";
$header_subtitle = "Viewing all canonical confirmation records in the Diocese.";
...
$records = db_fetchAll("
    ...
    $where
    ORDER BY c.date_of_confirmation DESC
", $params);
...
<head>
    ...
    <title>Confirmation Registry - Hwange Diocesan RMS</title>
                ...
                <div class="card bg-card no-padding overflow-hidden" style="border-radius: 1.5rem; border: 1px solid rgba(255,255,255,0.05); backdrop-filter: blur(20px);">
                    <div class="card-header" style="padding: 1.5rem 2rem; display: flex; justify-content: space-between; align-items: center; background: rgba(255,255,255,0.02);">
                        <div style="display: flex; align-items: center; gap: 2rem; flex: 1;">
                            ...
                            <!-- Registry Search Bar -->
                            <form method="GET" action="confirmation_list.php" style="flex: 1; max-width: 500px; position: relative; display: flex; gap: 10px;">
                                ...
                                <?php if ($search): ?>
                                    <a href="confirmation_list.php" class="btn btn-secondary" style="padding: 0 1rem; border-radius: 0.75rem; display: flex; align-items: center; font-size: 0.75rem;">Clear</a>
                                <?php endif; ?>
                            </form>
                        </div>
                        <div class="table-actions">
                            <a href="confirmation_add.php" class="btn btn-primary" style="background: #ef4444; color: white; font-weight: 800; display: flex; align-items: center; gap: 8px; border: none; box-shadow: 0 10px 20px rgba(239, 68, 68, 0.2);">
                        ...
                        <table class="data-table" style="width: 100%; border-collapse: collapse;">
                            <thead>
                                <tr style="background: rgba(255,255,255,0.02); text-align: left;">
                                    ...
                                    <th style="padding: 1.25rem 2rem; font-size: 0.75rem; color: var(--text-muted); text-transform: uppercase; letter-spacing: 1px;">Confirmed (Full Name)</th>
                                    ...
                                    <th style="padding: 1.25rem 2rem; font-size: 0.75rem; color: var(--text-muted); text-transform: uppercase; letter-spacing: 1px;">Confirmation Date</th>
                                    <th style="padding: 1.25rem 2rem; font-size: 0.75rem; color: var(--text-muted); text-transform: uppercase; letter-spacing: 1px;">Sponsor</th>
                                ...
                                <?php if (empty($records)): ?>
                                    <tr>
                                        <td colspan="6" style="padding: 4rem; text-align: center; color: var(--text-muted);">
                                            <ion-icon name="flame-outline" style="font-size: 3rem; margin-bottom: 1rem; opacity: 0.2;"></ion-icon>
                                            <p>No confirmation records found in the registry.</p>
                                        </td>
                                    </tr>
                                <?php else: ?>
                                    <?php foreach ($records as $row): ?>
                                        <tr style="border-bottom: 1px solid rgba(255,255,255,0.03); transition: background 0.3s;" onmouseover="this.style.background='rgba(255,255,255,0.02)'" onmouseout="this.style.background='transparent'">
                                            <td style="padding: 1.25rem 2rem; font-family: monospace; font-size: 0.85rem; color: var(--accent);"><?php echo h($row['entry_number'] ?: 'C-'.str_pad($row['confirmation_id'], 4, '0', STR_PAD_LEFT)); ?></td>
                                            <td style="padding: 1.25rem 2rem;">
                                                <div style="font-weight: 700; color: white;"><?php echo h($row['first_name'] . ' ' . $row['last_name']); ?></div>
                                                <div style="font-size: 0.75rem; color: var(--text-muted);">Minister: <?php echo h($row['minister']); ?></div>
                                            </td>
                                            <td style="padding: 1.25rem 2rem; font-size: 0.9rem; color: var(--text-muted);"><?php echo h($row['parish_name']); ?></td>
                                            <td style="padding: 1.25rem 2rem; font-size: 0.9rem;"><?php echo date('d M Y', strtotime($row['date_of_confirmation'])); ?></td>
                                            <td style="padding: 1.25rem 2rem; font-size: 0.9rem; color: var(--text-muted);"><?php echo h($row['sponsor'] ?: '-'); ?></td>
                                            <td style="padding: 1.25rem 2rem;">
                                                <div style="display: flex; gap: 0.75rem;">
                                                    <a href="confirmation_certificate.php?id=<?php echo $row['confirmation_id']; ?>" target="_blank" title="Download Certificate" style="color: #ef4444; font-size: 1.25rem;"><ion-icon name="print-outline"></ion-icon></a>
                                                    <a href="view_confirmation.php?id=<?php echo $row['confirmation_id']; ?>" title="View Full Details" style="color: white; font-size: 1.25rem;"><ion-icon name="eye-outline"></ion-icon></a>
                                                    <?php if (is_admin()): ?>
                                                        <a href="../actions/delete_record.php?type=confirmation&id=<?php echo $row['confirmation_id']; ?>" 

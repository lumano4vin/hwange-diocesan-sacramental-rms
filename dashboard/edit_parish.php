 ...
 * Parish Directory - Edit Parish/Mission
 */
...
// Fetch Parish ID
$id = $_GET['id'] ?? 0;
$parish = db_fetch("SELECT * FROM parishes WHERE parish_id = ?", [$id]);

if (!$parish) {
    die("Error: Parish record not found.");
}
...
<head>
    ...
    <title>Edit Parish - Hwange Diocese RMS</title>
...
<body class="dashboard-body">
    ...
    <div class="dashboard-layout" id="app-layout">
        ...
        <main class="main-content">
            ...
            <?php 
                $header_title = "Edit Parish: " . h($parish['parish_name']);
                $header_subtitle = "Update canonical or contact details for this mission/parish.";
                ob_start(); ?>
            ...
            <form action="../actions/save_parish.php" method="POST" class="entry-form">
                <input type="hidden" name="parish_id" value="<?php echo $parish['parish_id']; ?>">
                
                <div class="form-grid single-column">
                    
                    <div class="card bg-card section-card">
                        <div class="card-header">
                            <h3><ion-icon name="business-outline"></ion-icon> Update Information</h3>
                        </div>
                        
                        <div class="form-group">
                            <label for="parish_name">Official Parish Name</label>
                            <input type="text" name="parish_name" id="parish_name" value="<?php echo h($parish['parish_name']); ?>" required>
                        </div>
                        ...
                        <div class="form-row">
                            <div class="form-group">
                                ...
                                <select name="deanery" id="deanery" required>
                                    <option value="Binga" <?php if ($parish['deanery'] == 'Binga') echo 'selected'; ?>>Binga</option>
                                    <option value="Dete" <?php if ($parish['deanery'] == 'Dete') echo 'selected'; ?>>Dete</option>
                                    <option value="Hwange Urban" <?php if ($parish['deanery'] == 'Hwange Urban') echo 'selected'; ?>>Hwange Urban</option>
                                    <option value="Lupane" <?php if ($parish['deanery'] == 'Lupane') echo 'selected'; ?>>Lupane</option>
                                    <option value="Jambezi" <?php if ($parish['deanery'] == 'Jambezi') echo 'selected'; ?>>Jambezi</option>
                                    <option value="Makwa" <?php if ($parish['deanery'] == 'Makwa') echo 'selected'; ?>>Makwa</option>
                                    <option value="Victoria Falls" <?php if ($parish['deanery'] == 'Victoria Falls') echo 'selected'; ?>>Victoria Falls</option>
                                </select>
                            </div>
                            <div class="form-group">
                                <label for="location">Physical Location</label>
                                <input type="text" name="location" id="location" value="<?php echo h($parish['location']); ?>" required>
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="priest_in_charge_id" id="priest_in_charge_id">
                                ...
                                <option value="<?php echo $p['user_id']; ?>" <?php if ($parish['priest_in_charge_id'] == $p['user_id']) echo 'selected'; ?>>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="contact_number" id="contact_number" value="<?php echo h($parish['contact_number']); ?>">
                        </div>

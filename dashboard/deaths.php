...
$parish_filter_sql = get_parish_filter($params, 'd');
...
// Fetch Deaths joined with Parishioners and Parishes
$sql = "SELECT d.*, p.first_name, p.last_name, p.dob, pr.parish_name 
        FROM deaths d
...
<head>
    ...
    <title>Death Registry - Hwange Diocese RMS</title>
...
<body class="dashboard-body">
    ...
    <div class="dashboard-layout" id="app-layout">
        ...
        <main class="main-content">
            ...
            <?php 
                $header_title = "Death Registry";
                $header_subtitle = "Record and search for the final milestones of the faithful.";
                ob_start(); ?>
                <a href="add_death.php" class="btn btn-primary">
            ...
            <div class="filter-bar">
                <form action="deaths.php" method="GET" class="filter-group" id="filter-form">
                    <div class="search-input-group" style="min-width: 300px;">
                        <ion-icon name="search-outline"></ion-icon>
                        <input type="text" name="q" value="<?php echo h($search); ?>" placeholder="Search name...">
                    </div>
                    ...
                    <?php if(!empty($search) || !empty($year_filter) || !empty($parish_filter)): ?>
                        <a href="deaths.php" class="btn btn-secondary btn-sm" title="Clear Filters" style="padding: 0.5rem;">
                        ...
                        <tbody>
                            ...
                            <?php else: ?>
                                ...
                                <tr>
                                    ...
                                    <td><?php echo h($row['date_of_death']); ?></td>
                                    <td><?php echo h($row['date_of_burial']); ?></td>
                                    <td><?php echo h($row['place_of_burial']); ?></td>
                                    ...
                                    <td class="table-actions">
                                        <a href="view_death.php?id=<?php echo $row['death_id']; ?>" class="action-btn view" title="View Details">
                                            <ion-icon name="eye-outline"></ion-icon>
                                        </a>

                                        <?php if (has_record_permission($row, 'print')): ?>
                                        <a href="death_certificate.php?id=<?php echo $row['death_id']; ?>" class="action-btn cert" title="View Certificate">
                                            <ion-icon name="document-text-outline"></ion-icon>
                                        </a>
                                        <a href="death_certificate.php?id=<?php echo $row['death_id']; ?>&print=true" class="action-btn" title="Print Certificate" target="_blank">
                                            <ion-icon name="print-outline"></ion-icon>
                                        </a>
                                        <?php endif; ?>

                                        <?php if (has_record_permission($row, 'edit')): ?>
                                        <a href="edit_death.php?id=<?php echo $row['death_id']; ?>" class="action-btn edit" title="Edit">
